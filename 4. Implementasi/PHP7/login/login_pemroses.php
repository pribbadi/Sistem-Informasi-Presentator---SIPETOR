<?php 
	...
	if(isset($_POST['username']) AND ($_POST['password'])){ //isset= mengecek apakah ada data yang di masukkan dari inputan username dan password
		...
		$result = mysqli_query($link, $sql);//membuatvariabel result untuk melihat semua data dari tabel admin
		$jumlahditemukan=mysqli_num_rows($result);
		...
		$resultmhs = mysqli_query($link, $sqlmhs);//membuatvariabel result untuk melihat semua data dari tabel admin
		$jumlahditemukanmhs=mysqli_num_rows($resultmhs);

		if ($jumlahditemukan == 1){ //kondisi :melihat data per rows dari tabel admin apakah ada(1) atau tidak
		  	$row = mysqli_fetch_array($result);//melihatkan perintah query berdasarkan objek oriented
		  	...
		  	header("location:../admin/halamanutama/");//  meredirect ke kehalaman admin/index.php
	 	  	...
	 	  	$rowmhs = mysqli_fetch_array($resultmhs);//melihatkan perintah query berdasarkan objek oriented
